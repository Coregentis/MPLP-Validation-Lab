...
export function evaluateCLD1_03(bundle: RunBundle): ClauseResult {
    ...
    // Refined Semantic Check (v0.13.1):
    // For deny/throttle, we strictly require evidence of enforcement in the trace.
    // Enforcement events must be of type 'gate', 'block', 'stop', or 'budget.enforcement'.
    const events = getAllEvents(bundle);
    const enforcementEvents = events.filter(e => {
        const et = normalizeToken(e.event_type);
        return et.includes('gate') || et.includes('block') || et.includes('stop') || et.includes('enforcement') || et.includes('throttle');
    });

    if (enforcementEvents.length === 0) {
        notes.push(`Deny/throttle (${fields.outcome}) but no engineering enforcement evidence detected in trace`);
        return createClauseResult(def, 'FAIL', evidence, notes, 'D1_BUDGET_DENY_WITHOUT_GATE');
    }

    notes.push(`Enforcement evidence verified: found ${enforcementEvents.length} gate/enforcement event(s)`);
    return createClauseResult(def, 'PASS', evidence, notes);
}
