#!/usr/bin/env bash
# Smoke test for python-3.11 runner
...
echo "=== Python-3.11 Runner Smoke Test ==="
...
"$SCRIPT_DIR/../_lib/run-in-container.sh" \
  --runner "$RUNNER_ID" \
  --cmd "python --version && poetry --version && echo 'smoke-test-pass' > out/smoke.txt" \
...
echo "✅ PASS: Python-3.11 runner smoke test"
